...
(define-datatype expression expression?  
    [var-exp
        (id symbol?)] 
    [lambda-exp
        (id (list-of (lambda (x) (or (symbol? x) (and (pair? x) (eqv? 'ref (car x)))))))
...
(define extend-env
    (lambda (syms vals env)
        (extended-env-record syms (map (lambda (x)
                                        (if (cell? x)
                                            x
                                            (cell x))) vals) env)))
    ...
    (cases expression exp
        ...
        [app-exp (rator rands)
           (let ([proc-value (eval-exp rator env)]
                 ...
                 [ref-lst (get-ref-lst rands env)])
                (apply-proc proc-value args ref-lst))]
        [if-exp (condition tcase other)
        ...
        [define-exp (id exp)
            (set! init-env (extend-env (list id) (list (eval-exp exp init-env)) init-env))]
           ...
           (if (eqv? 'var-exp (car x))
               (apply-env-ref env
                              ...
                              (apply-env-ref init-env
                                             (cadr x)
                                             (lambda (x) x)
                                             (lambda () (eopl:error 'get-ref-lst "variable not found in environment: ~s" (cadr x))))))))
         rands)))

(define update-ref
  (lambda (ids env ref-lst)
    (let loop ([ids ids] [index 0])
      (cond 
        [(null? ids) (void)]
        [(pair? (car ids))
          (begin (set-ref! (list-ref ref-lst index) (apply-env-ref env 
                                                                  (cadar ids)
                                                                  (lambda (x) (deref x))
                                                                  (lambda () (eopl:error 'update-ref "variable not found in environment: ~s" (cadar x)))))
                 (loop (cdr ids) (add1 index)))]
        [else (loop (cdr ids) (add1 index))]))))
...
(define apply-proc
    (lambda (proc-value args ref-lst)
        (cases proc-val proc-value
            [prim-proc (op) (apply-prim-proc op args)]
			; You will add other cases
            [closure (ids bodies env)
                (let* ([new-env (extend-env (map (lambda (x) 
                                                    (if (pair? x) 
                                                        (cadr x) 
                                                        x)) 
                                                 ids) 
                ...
                (update-ref ids new-env ref-lst)
                result)]
