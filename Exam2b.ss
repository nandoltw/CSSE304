; CSSE304-01 Yuankai Wang (Kevin) Exam2b
        ...
        (cond
            ...
            [(pair? datum)
                (cond
                    ...
                    [(number? (1st datum))
                        (lit-exp datum)]
                    [(equal? (1st datum) 'trace-lambda); change
                        (trace-lambda-exp (2nd datum) (3rd datum) (parse-exp (cadddr datum)))]
...
(define eval-exp
    ...
    (cases expression exp
        ...
        ; change
        ; I will need to first get the value of the varible in id. Then display it. Then run it just like normal lambda with the variable.
        ; Then display the result.
        [trace-lambda-exp (name id body)
        	(set! indent (+ indent 1))
        	(begin (eval-exp (syntax-expand (lambda-exp id body)))
        			(display-traced-output indent name id))
        	(let ([temp (eval-exp body env)])
	        	(display-traced-output indent temp))
        	(set! indent (- indent 1))]
