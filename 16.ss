                ...
                (let ([pos (list-find-position sym procnames)])
                    (if (number? pos)
                        (if (list? (list-ref idss pos))
                            ...
                            (closure-pair (append (list (car (list-ref idss pos))) (list (cdr (list-ref idss pos)))) (list-ref bodiess pos) env))
    ...
    (lambda (exp)
        (cases expression exp
            ...
            [let-name-exp (name syms exps bodies)
                (letrec-exp (list name) (list syms) (list bodies) (list (app-exp (var-exp name) exps)))]
            [let*-exp (syms exps bodies)
